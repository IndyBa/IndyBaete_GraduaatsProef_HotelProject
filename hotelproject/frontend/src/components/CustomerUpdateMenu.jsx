...
const CustomerUpdateMenu = ({ handleUpdateCustomerClose, showUpdateCustomerMenu, customer }) => {
  const { customerData, isCustomerLoading, isCustomerError, customerError } = customer ?? {};
  const { id, name, email, phone, municipality, zipCode, street, houseNumber  } = customerData ?? {};

  const { isError, error, mutate, isSuccess, isPending, reset } = useMutation({
    mutationKey: ["updateCustomer"],
    mutationFn: (updateData) => updateCustomer(updateData.values, updateData.id),
  });
  ...
  return (
    <div className="model_box">
      <Modal
        ...
        onHide={handleUpdateCustomerClose}
        backdrop="static"
        keyboard={false}
      ...
      >
        ...
        <Modal.Body>
          ...
          </form>) : isCustomerError ? <h2>Error with loading customer: {customerError.message}</h2> : <h2>Loading customer...</h2>
          }
        </Modal.Body>
        <Modal.Footer>
